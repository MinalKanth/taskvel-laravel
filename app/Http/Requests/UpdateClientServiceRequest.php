...
class UpdateClientServiceRequest extends FormRequest
{
    ...
    {
        $clientServiceId = $this->route('service')?->id
            ?? $this->route('client_service')?->id
            ?? $this->route('client_service');
        ...
        return [
            ...
            */
            ...
            'client_id' => [
                ...
                Rule::unique('client_services')
                    ->where(fn ($query) => $query->where(
                        'service_id',
                        $this->input('service_id')
                    ))
                    ->ignore($clientServiceId),
            ],
    ...
    public function messages(): array
    {
        return [
            ...
            'client_id.unique' => 'This service is already assigned to the selected client.',
